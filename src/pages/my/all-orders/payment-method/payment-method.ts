...
export class PaymentMethodPage {
   ...
   goAllOrdersPage() {
      ...
      if (this.navCtrl.getPrevious() && (this.navCtrl.getPrevious().id == 'AllOrdersPage' || this.navCtrl.getPrevious().id == 'OrderListDistributionPage')) {
         return this.navCtrl.pop().catch(res => { history.back() });
      } else {
         if (this.isDistribution) {
            return this.pushPage('OrderListDistributionPage');
         } else {
            return this.pushPage('AllOrdersPage');
         }
      }
   }
         ...
         this.httpService.FmCreditgate_order({ order_id: this.order_id }).then(res => {
            if (res.status) {
               ...
               this.goAllOrdersPage().then(() => {
                  ...
                  // this.iab.create('https://www.jingku.cn/openFmCredit.html?data=' + encodeURIComponent(res.data) + '&url=' + encodeURIComponent(res.api_url), this.native.isMobile() ? '_system' : '_self');
               });
            }
         })
      } else {//不使用余额
