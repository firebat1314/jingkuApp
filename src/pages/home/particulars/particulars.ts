...
export class ParticularsPage {
  ...
  presentModal(str, evt) {
    ...
    let modal = this.modalCtrl.create(ParticularsModalPage, { name: str, getBonus: this.getBonus });
  ...
  ngOnInit() {
    this.http.getGoodsGallery({ goods_id: 3994 }).then((res) => {
      console.log("商品详情的相册图片轮播", res);
      this.getGoodsGallery = res.data;
    });
    this.http.getPriceSection({ goods_id: 3994 }).then((res) => {
      console.log("获取商品价格优惠区间", res);
      this.getPriceSection = res;
    });
    this.http.getGoodsInfo({ goods_id: 3994 }).then((res) => {
      console.log("商品详情信息", res);
      this.getGoodsInfo = res.data;
    });
    this.http.getGoodsParameter({ goods_id: 3994 }).then((res) => {
      console.log("获取商品参数", res);
      this.getGoodsParameter = res.data;
      // console.log(JSON.stringify(res.data))
    });
    this.http.getGoodsSaleCity({ goods_id: 3994 }).then((res) => {
      console.log("获取商品的销售区域", res);
      this.getGoodsSaleCity = res.data;
    });
    this.http.getSupplierInfo({ goods_id: 3994 }).then((res) => {
      console.log("获取供应商信息", res);
      this.getSupplierInfo = res.data;
    });
    this.http.getBonus({ goods_id: 3994 }).then((res) => {
      console.log("优惠券列表", res);
      this.getBonus = res.data;
    });
    this.http.sendByUser({ goods_id: 3994 }).then((res) => {
      console.log("领取优惠券", res);
      this.sendByUser = res.data;
    });
    this.http.getGoodsFittings({ goods_id: 3994 }).then((res) => {
      console.log("组合商品、关联商品", res);
      this.getGoodsFittings = res.data;
    });
    this.http.searchGoods({ goods_id: 3994 }).then((res) => {
