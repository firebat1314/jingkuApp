...
/*
  Generated class for the CustomeServicesProvider provider.

  See https://angular.io/guide/dependency-injection for more info on providers
  and Angular DI.
*/
@Injectable()
export class CustomeServicesProvider {
   ...
   constructor(
      ...
      //表情对象，用户可以自定义
      webim.Emotions = {
         "0": ["[):]", '../assets/icon/emotions/ee_1.png'],
         "1": ["[:D]", '../assets/icon/emotions/ee_2.png'],
         "2": ["[;)]", '../assets/icon/emotions/ee_3.png'],
         "3": ["[:-o]", '../assets/icon/emotions/ee_4.png'],
         "4": ["[:p]", '../assets/icon/emotions/ee_5.png'],
         "5": ["[(H)]", '../assets/icon/emotions/ee_6.png'],
         "6": ["[:@]", '../assets/icon/emotions/ee_7.png'],
         "7": ["[:s]", '../assets/icon/emotions/ee_8.png'],
         "8": ["[:$]", '../assets/icon/emotions/ee_9.png'],
         "9": ["[:(]", '../assets/icon/emotions/ee_10.png'],
         "10": ["[:'(]", '../assets/icon/emotions/ee_11.png'],
         "11": ["[:|]", '../assets/icon/emotions/ee_18.png'],
         "12": ["[(a)]", '../assets/icon/emotions/ee_13.png'],
         "13": ["[8o|]", '../assets/icon/emotions/ee_14.png'],
         "14": ["[8-|]", '../assets/icon/emotions/ee_15.png'],
         "15": ["[+o(]", '../assets/icon/emotions/ee_16.png'],
         "16": ["[<o)]", '../assets/icon/emotions/ee_12.png'],
         "17": ["[|-)]", '../assets/icon/emotions/ee_17.png'],
         "18": ["[*-)]", '../assets/icon/emotions/ee_19.png'],
         "19": ["[:-#]", '../assets/icon/emotions/ee_20.png'],
         "20": ["[:-*]", '../assets/icon/emotions/ee_22.png'],
         "21": ["[^o)]", '../assets/icon/emotions/ee_21.png'],
         "22": ["[8-)]", '../assets/icon/emotions/ee_23.png'],
         "23": ["[(|)]", '../assets/icon/emotions/ee_24.png'],
         "24": ["[(u)]", '../assets/icon/emotions/ee_25.png'],
         "25": ["[(S)]", '../assets/icon/emotions/ee_26.png'],
         "26": ["[(*)]", '../assets/icon/emotions/ee_27.png'],
         "27": ["[(#)]", '../assets/icon/emotions/ee_28.png'],
         "28": ["[(R)]", '../assets/icon/emotions/ee_29.png'],
         "29": ["[({)]", '../assets/icon/emotions/ee_30.png'],
         "30": ["[(})]", '../assets/icon/emotions/ee_31.png'],
         "31": ["[(k)]", '../assets/icon/emotions/ee_32.png'],
         "32": ["[(F)]", '../assets/icon/emotions/ee_33.png'],
         "33": ["[(W)]", '../assets/icon/emotions/ee_34.png'],
         "34": ["[(D)]", '../assets/icon/emotions/ee_35.png']
      };
   }
   ...
   onKickedEventCall() {
      // this.native.openAlertBox('被其他登录实例踢下线',()=>{})
   ...
   getLastC2CHistoryMsgs = (cbOk, cbError) => {
      if (this.selType == webim.SESSION_TYPE.GROUP) {
         this.native.showToast('当前的聊天类型为群聊天，不能进行拉取好友历史消息操作');
         return;
      }
      if (!this.selToID || this.selToID == '@TIM#SYSTEM') {
         this.native.showToast('当前的聊天id非法，selToID=' + this.selToID);
         return;
      }
   ...
   getPrePageC2CHistoryMsgs = (cbOk?, cbError?) => {
      ...
      if (tempInfo) {
         ...
         this.native.showToast('获取下一次拉取的c2c消息时间和消息Key为空');
         return;
      }
      let options = {
         'Peer_Account': this.selToID, //好友帐号
   ...
   getLastGroupHistoryMsgs = (cbOk, cbErr) => {
      if (this.selType == webim.SESSION_TYPE.C2C) {
         this.native.showToast('当前的聊天类型为好友聊天，不能进行拉取群历史消息操作');
         return;
      }
      ...
      webim.getGroupInfo(
         ...
         (resp) => {
            if (resp.GroupInfo[0].ShutUpAllMember == 'On') {
               this.native.showToast('该群组已开启全局禁言');
            }
            if (cbOK) {
               cbOK(resp);
            }
   ...
   onSendMsg(msgContent, cbOK?) {
      ...
      /* if (!this.selToID) {
         this.native.showToast("你还没有选中好友或者群组，暂不能聊天");
         return;
      } */
      //获取消息内容
   ...
   sendPic(images, imgName) {
      ...
      if (!this.selToID) {
         this.native.showToast("您还没有好友，暂不能聊天");
         return;
      }
   ...
   checkPic(obj, fileSize) {
      ...
      if (pos < 0) {
         this.native.showToast("您选中的文件不是图片，请重新选择");
         return false;
      }
      fileSize = Math.round(fileSize / 1024 * 100) / 100; //单位为KB
      if (fileSize > 30 * 1024) {
         this.native.showToast("您选择的图片大小超过限制(最大为30M)，请重新选择");
         return false;
      }
      return true;
   }
   ...
   selectFileClick() {
      ...
      if (webim.BROWSER_INFO.type == 'ie' && parseInt(webim.BROWSER_INFO.ver) <= 9) {
         this.native.showToast('上传文件暂不支持ie9(含)以下浏览器');
      } else {
      }
   }
   ...
   checkFile(file) {
      ...
      //if (pos < 0) {
      //    this.native.showToast("您选中的文件类型非法，请重新选择");
      //    return false;
      //}
      let fileSize = Math.round(file.size / 1024 * 100) / 100; //单位为KB
      if (fileSize > 20 * 1024) {
         this.native.showToast("您选择的文件大小超过限制(最大为20M)，请重新选择");
         return false;
      }
      return true;
   }
